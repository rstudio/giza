...
(function(){
  ...
  this.subscribe = function(filter){
    if (filter.match(/^\//)){
      // Must be a path. Just pass through
      this.$bubbler.subscribe.apply(this.$bubbler, arguments);
    } else if (filter.match(/^type=/)){
      var paths = this.$store.getNodesOfType(filter.match(/^type=(.*)$/)[1]);
      _.each(paths, function(path){
        var args = Array.prototype.slice.call(arguments);
        args[0] = path;
        this.$bubbler.subscribe.apply(this.$bubbler, args);
      });
    } else{
      throw new Error("Can't understand subscription filter: " + filter);
    }
  }
