...
var gizaLogger = require('./core/log'),
  ...
  Blacklist = require('./assemblers/blacklist'),
  Whitelist = require('./assemblers/whitelist');
...
function Giza(assemblers){
  ...
  this.$bubbler = new Bubbler(null, logger);
  this.$store = new Store(this.$bubbler, this.$assemblers, logger);
...
(function(){
  ...
  this.get = function(path, options){
    ...
    if (options.callback){
      this.$bubbler.subscribe(path, options.callback, {bubble: recursive});
    }
