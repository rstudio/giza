...
(function(){
  ...
  this.emit = function(path, event){
    ...
    // emit locally
    this.$eventBus.emit(path, event, path);
 ...
 * time spent bubbling).
 */
function bubbleEvent(path, event, bus, source){
  // No source given. Assume the current path is the root
  if (arguments.length < 4 || typeof source === 'undefined'){
    source = path;
  }
  ...
  if (lastSlash !== -1){
    ...
    if (path === ''){
      ...
      bus.emit(path, event, source);
      bubbleEvent(path, event, bus, source);
    }
  }
}